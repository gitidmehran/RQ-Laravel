...
use App\Models\NonPubAyatsTranslation;
use App\Models\NonPubWordsTranslations;
use App\Models\NonPubWordReferences;
use App\Models\NonPubPhrasesWords;
use App\Models\NonPubOtherWordsInfo;
use App\Models\NonPubWordNotes;
use App\Models\NonPubPhrasesWordsTranslations;
...
class TranslationController extends Controller
{
    ...
    try{
            ...
            $response = [
                   'success' => true,
                   'message' => 'Translations Data Retrieved Successfully',
                   'data' => $scholars
                ];
        } catch (\Exception $e) {
    ...
    public function getTranslation(Request $request){
        try{
            ...
            $ayat = InfoData::with(
                'words.singleReferenceWord.word.ayat:id,surah_id,ayatNo',
                'words.phraseReferenceWord.word.ayat:id,surah_id,ayatNo', 
                'words.phrasesWords'
                )
                ->where(['surah_id'=>$input['surahNo'],'ayatNo'=>$input['ayat']])
                ->get()->first()->toArray();
            ...
            if($isAlreadyAdded > 0){
                ...
                'message'=> "You have already added,You Can't add Duplicate Translation.",
                'authId'=>$authId,
                'ayat_id'=>$ayat['id']);
                return response()->json($response);
            }
...
//return response()->json($params1);
        ...
        $word_reference = WordReferences::where('scholar_id',$scholarId)->get()->toArray();
        
        $word_reference_ids = array_unique(array_column($word_reference,'reference_word_id')); 

        $phrase_word_reference = PhrasesWords::where('scholar_id',$scholarId)->get()->toArray();
        ...
        if ( $t_w > 1 ) {
            ...
            $data = [
                'success' => true,
                'message'         => 'Phrase Word References',
    ...
    $response = [
               'success' => true,
               'message' => 'Word References Data Retrieved Successfully',
               'data' => $data
            ];
        } catch (\Exception $e) {
            $response = array('success'=>false,'message'=>$e->getMessage());
    ...
    //return response()->json("rizwan");
        $input = $request->all();
      //return response()->json($input);  
        $checkAlreadyExist = AyatsTranslation::where(['ayat_id'=>$input['ayat_id'],'scholar_id'=>$input['auth_id']])->count();
    ...
    //return response()->json($isApproved);        
        try{
            ...
            if($isApproved != 2){
                ...
                // INSERT RECORDS IF NOT EMPTY 
                if(!empty($requestdata['ayat_translations'])) 
                    NonPubAyatsTranslation::insert($requestdata['ayat_translations']);
//print_r($requestdata['word_translations']);exit;
                if(!empty($requestdata['word_translations'])) 
                    NonPubWordsTranslations::insert($requestdata['word_translations']);

                if(!empty($requestdata['reference_word_translations']))
                    NonPubWordsTranslations::insert($requestdata['reference_word_translations']);

                if(!empty($requestdata['word_preferences_array'])) 
                    NonPubWordReferences::insert($requestdata['word_preferences_array']);

                if(!empty($requestdata['word_numbers_array'])) 
                    NonPubPhrasesWords::insert($requestdata['word_numbers_array']);

                if(!empty($requestdata['phrase_reference_word_translations'])) 
                    NonPubPhrasesWordsTranslations::insert($requestdata['phrase_reference_word_translations']);

                if(!empty($requestdata['otherinfo_array'])) 
                    NonPubOtherWordsInfo::insert($requestdata['otherinfo_array']);

                if(!empty($requestdata['notes_array'])) 
                    NonPubWordNotes::insert($requestdata['notes_array']);
            }
            DB::commit();
            \Session::put('word_references',[]);
            \Session::put('phrase_word_references',[]);
            $response = array('success'=>true,'message'=>'Data Save Successfully');
        }catch(\Exception $e){
            DB::rollback();            
            $response = array('success'=>false,'message'=>$e->getMessage());
        }
    ...
    public function translationsByScholars(Request $request){
        try{
            $authId = $request->has('scholar_id')?$request->get('scholar_id'):Auth::id();
            $column = 'created_user';
    ...
    try{
        ...
        foreach ($languages as $key => $value) {
            ...
            $ayat_translations[$key]['scholar_id'] = $authId;
            ...
            {    
                ...
                if(!empty($requestdata['ayat_translations'])){
                    foreach ($requestdata['ayat_translations'] as $key => $val) {
                        $isExist = AyatsTranslation::where(['ayat_id'=>$val['ayat_id'],'scholar_id'=>$val['scholar_id'],'language'=>$val['language']])->first();
                        if(!empty($isExist)){
                ...
                if(!empty($requestdata['word_translations'])){
                    WordsTranslations::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    WordsTranslations::insert($requestdata['word_translations']);
                }

                if(!empty($requestdata['reference_word_translations'])){
                    $references = WordReferences::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->get()->toArray();
                    if(!empty($references)){
                        $referencesIds = array_column($references,'reference_word_id');
                        WordsTranslations::where(['scholar_id'=>$input['auth_id'],'is_reference_word'=>1])->whereIn('word_id',$referencesIds)->delete();
                    }
                    WordsTranslations::insert($requestdata['reference_word_translations']);
                }

                if(!empty($requestdata['word_preferences_array'])){
                    WordReferences::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    WordReferences::insert($requestdata['word_preferences_array']);
                }

                if(!empty($requestdata['word_numbers_array'])){
                    PhrasesWords::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    PhrasesWords::insert($requestdata['word_numbers_array']);                
                }

                if(!empty($requestdata['phrase_reference_word_translations'])) 
                    $references = PhrasesWords::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->get()->toArray();
                    // echo json_encode( $references );exit;

                    if(!empty($references)){
                        $referencesIds = array_column($references,'phrase_word_id');
                        PhrasesWordsTranslations::where(['scholar_id'=>$input['auth_id'],'is_reference_word'=>1])->whereIn('word_id',$referencesIds)->delete();
                    }
                    PhrasesWordsTranslations::insert($requestdata['phrase_reference_word_translations']);

                if(!empty($requestdata['otherinfo_array'])){
                    OtherWordsInfo::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    OtherWordsInfo::insert($requestdata['otherinfo_array']);
                }

                if(!empty($requestdata['notes_array'])){
                    foreach ($requestdata['notes_array'] as $key => $val) {
                        WordNotes::where(['word_id'=>$val['word_id'],'scholar_id'=>$val['scholar_id'],'note_id'=>$val['note_id']])->delete();
                        WordNotes::insert($val);
                    }
                }
        }else{    
//for Non Publish Users
                // INSERT OR UPDATE RECORDS IF NOT EMPTY 
                if(!empty($requestdata['ayat_translations'])){

//return response()->json($requestdata['ayat_translations']);
                    foreach ($requestdata['ayat_translations'] as $key => $val) {
 //return response()->json($val);    
                        $isExist = NonPubAyatsTranslation::where(['ayat_id'=>$val['ayat_id'],'scholar_id'=>$val['scholar_id'],'language_id'=>$val['language_id']])->first();
                     // return response()->json($isExist);    
                        if(!empty($isExist)){
                            ...
                            //return response()->json("isExist");
                            NonPubAyatsTranslation::insert($val);
                        }
                    }
                }

                if(!empty($requestdata['word_translations'])){
                    NonPubWordsTranslations::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    NonPubWordsTranslations::insert($requestdata['word_translations']);
                }

                if(!empty($requestdata['reference_word_translations'])){
                    $references = NonPubWordReferences::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->get()->toArray();
                    if(!empty($references)){
                        $referencesIds = array_column($references,'reference_word_id');
                        NonPubWordsTranslations::where(['scholar_id'=>$input['auth_id'],'is_reference_word'=>1])->whereIn('word_id',$referencesIds)->delete();
                    }
                    NonPubWordsTranslations::insert($requestdata['reference_word_translations']);
                }

                if(!empty($requestdata['word_preferences_array'])){
                    NonPubWordReferences::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    NonPubWordReferences::insert($requestdata['word_preferences_array']);
                }

                if(!empty($requestdata['word_numbers_array'])){
                    NonPubPhrasesWords::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    NonPubPhrasesWords::insert($requestdata['word_numbers_array']);                
                }

                if(!empty($requestdata['phrase_reference_word_translations'])) 
                    $references = NonPubPhrasesWords::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->get()->toArray();
                    // echo json_encode( $references );exit;

                    if(!empty($references)){
                        $referencesIds = array_column($references,'phrase_word_id');
                        NonPubPhrasesWordsTranslations::where(['scholar_id'=>$input['auth_id'],'is_reference_word'=>1])->whereIn('word_id',$referencesIds)->delete();
                    }
                    NonPubPhrasesWordsTranslations::insert($requestdata['phrase_reference_word_translations']);

                if(!empty($requestdata['otherinfo_array'])){
                    NonPubOtherWordsInfo::where('scholar_id',$input['auth_id'])->whereIn('word_id',$input['word_ids'])->delete();
                    NonPubOtherWordsInfo::insert($requestdata['otherinfo_array']);
                }

                if(!empty($requestdata['notes_array'])){
                    foreach ($requestdata['notes_array'] as $key => $val) {
                        NonPubWordNotes::where(['word_id'=>$val['word_id'],'scholar_id'=>$val['scholar_id'],'note_id'=>$val['note_id']])->delete();
                        NonPubWordNotes::insert($val);
                    }
                }
            ...
            //return redirect('/dashboard/'.$input['previous_url'])->with('success','Data Updated Successfully');
        }catch(\Exception $e){
            DB::rollback(); 
...
//echo json_encode( $word_references );exit;
    ...
    public function singleTranslationView(Request $request){
        ...
        $ayat = InfoData::with('words.singleReferenceWord.word.ayat:id,surah_id,ayatNo')->where('id',7)->get()->first()->toArray();
        return response()->json($ayat);
        \Session::put('word_references',[]);
...
return response()->json($data);
    ...
    public function removePreference(Request $request){
        try {
            $input = $request->all();
            WordsTranslations::where(['word_id'=>$input['word'],'scholar_id'=>$input['scholar_id'],'is_reference_word'=>1])->delete();
            $check_parent_delete = WordReferences::where(['word_id'=>$input['word']])->where(['reference_word_id'=>$input['word']])->count();
            // return response()->json($check_parent_delete);
            ...
            'message'=>"Word Preference deleted Successfully");
        } catch (\Exception $e) {
            $response = array('success'=>false,'message'=>$e->getMessage());
        }
        return response()->json($response);
    ...
    public function removePhrasePreference(Request $request){
        try {
            ...
            PhrasesWordsTranslations::where(['scholar_id'=>$input['scholar_id'],'is_reference_word'=>1])->whereIn( 'word_id', $refWord )->delete();
            $check_parent_delete = PhrasesWords::where(['word_id' => $input['word']])->where(['scholar_id'=>$input['scholar_id']])->count();
            if( $check_parent_delete > 0 ) { 
                // references are deleted
                PhrasesWords::where(['word_id' => $input['word'], 'scholar_id'=>$input['scholar_id']])->delete();
            } else {
                // only child reference were deleted
                PhrasesWords::where('scholar_id', $input['scholar_id'])->whereIn( 'phrase_word_id', $refWord )->delete();
            }

            $response = ['success'=>true,
            'message'=>"Phrase Word Preference deleted Successfully"];
        } catch (\Exception $e) {
            $response = array('success'=>false,'message'=>$e->getMessage());
        }
        return response()->json($response);
    ...
    public function destroy(Request $request,$id){
        try {
            ...
            DB::beginTransaction();
            if(!empty($word_ids)){
                WordReferences::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
                PhrasesWords::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
                // PhrasesWordsTranslations::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
                WordNotes::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
                WordsTranslations::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
                OtherWordsInfo::where('scholar_id',$authId)->whereIn('word_id',$word_ids)->delete();
            }
    ...
    private function getTranslations($column,$authId,$request){
        ...
        $scholars = array_unique(array_column($translations,'scholar_id'));
    ...
    if($request->get('per_page')){
        if($request->has('per_page') && !empty($request->get('per_page')))
            $this->perPage = $request->get('per_page');
    }
        ...
        ])->whereIntegerInRaw('id',$ayatIds)->orderBy('surah_id')->orderBy('ayatNo')->paginate($this->perPage);

        $count = $query->total();
        $links = $query->appends($request->all())->links('pagination::bootstrap-5');
        $ayatTranslations = $query->toArray();        
//return response()->json($ayatTranslations['data']);   
        $list = $language_names = [];
